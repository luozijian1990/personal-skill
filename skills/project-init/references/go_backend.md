...
Replace `{module}` with the actual Go module path (e.g., `github.com/user/project`). If the user doesn't specify a module path, use the project directory name.
...
pkg/logger/          # 全局 logger 封装，使用 logger.Info/Error/Debug/Warn
```
...
- **新增接口**：在 `internal/handler/` 下添加，在 `cmd/main.go` 中注册路由
...
Use the following template, replacing `{project_name}` and `{module}` with actual values:
...
Initialize with `go mod init {module}`, then run `go mod tidy` to fetch dependencies.
